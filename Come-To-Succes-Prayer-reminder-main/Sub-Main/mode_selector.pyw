...
def timenow():
    while True:#Time Monitor
        ...
        if b==(salahtimes[4]):
            break
    with open(f"{dir_path1}\\Data\\TimeNow.dat") as time1:
        pickle.dump(b,time1)

if a2==1:
    timenow()
    webbrowser.open(fr'{dir_path}\Modes\Easy.py')
if a2==2:
    timenow()
    webbrowser.open(fr'{dir_path}\odes\Mediam.py')
if a2==3:
    timenow()
    webbrowser.open(fr'{dir_path}\Modes\Hard.py')
if a2==4:
    timenow()
    webbrowser.open(fr'{dir_path}\Modes\Extream.py')